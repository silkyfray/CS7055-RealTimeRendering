...
ShaderProgram* ShaderManager::getShaderProgram(const std::string &vertexShader, const std::string &fragmentShader, ShaderType shaderType)
{
	...
	if (it == loadedShaderPrograms.end())
	{
		...
		if(shaderType == ShaderType::GOURAUD)
			loadGouraudShader(newShader);
		else if(shaderType == ShaderType::PHONG)
			loadPhongShader(newShader);
		else if(shaderType == ShaderType::CEL)
			loadCelShader(newShader);
...
void ShaderManager::loadGouraudShader(ShaderProgram *shaderProgram)
{
