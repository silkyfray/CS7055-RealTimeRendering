...
void main()
{
    ...
    if(enableWobble)
    {
        ...
        if(yGradient > 0.0)
            ...
            yy = yGradient;
        col = vec3(texture(screenTexture, vec2(fsTexCoord.x - xGradient * offsetX, fsTexCoord.y - yGradient * offsetY)));
    }
    else
    {
    ...
    {        
        ...
        vec3 colDeltaX_RGB = vec3(texture(screenTexture, fsTexCoord.st + vec2(-pixelWidth, 0))) - vec3(texture(screenTexture, fsTexCoord.st + vec2(pixelWidth, 0)));
        float colDeltaX_Gray = abs((colDeltaX_RGB.r + colDeltaX_RGB.g + colDeltaX_RGB.b) * 0.3333);
        vec3 colDeltaY_RGB = vec3(texture(screenTexture, fsTexCoord.st + vec2(0, -pixelHeight))) - vec3(texture(screenTexture, fsTexCoord.st + vec2(0, pixelHeight)));
    ...
    if(enablePigment)
    {
        ...
        float pigmentDensity = 1.0 + scaleFactorPigment * (texture(pigmentTexture, fsTexCoord).r - 0.5);
        col = col - (col - vec3(col*col)) * (pigmentDensity - 1.0);
        //col = col * -pigmentDensity;
    }
    if(enableTurbulence)
    {
        float turbulenceDensity = 1 + scaleFactorTurbulence * (texture(turbulenceTexture, fsTexCoord).r - 0.5);
    ...
    if(enablePaper)
    {
        float paperDensity = 1 + scaleFactorPaper * (texture(paperTexture, fsTexCoord).r - 1.0);
        //col = col * paperDensity;
